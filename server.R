...
plot(km , xlab="Months", ylab='S(t)', conf.int = F, main = 'Kaplan Meyer Plot',xlim = c(0,441), lwd = 2, las = 1)
...
mods <- c("exponential","weibull","loglogistic",'lognormal','gompertz','gamma','genf')
...
server <- function(input, output) {
  ...
  # Fit a Kaplan-Meier model to the Breast cancer dataset
  km_fit <- survfit(Surv(Time, event) ~ 1, data = shiny_df)
  
  # Create a Kaplan-Meier plot
  output$km_plot <- renderPlot({
    plot(km_fit, xlab = "Time (Years)", ylab = "Survival Probability", las = 1, lwd = 3, conf.int = input$km_ci)
    
    # Add the predicted survival probabilities if the checkbox is checked
    if (input$show_expo) {
      
      # Fit an Exponential model via survival package
      psm_fit <- survreg(Surv(Time, event) ~ age + tumor_size + lnep + gcs_2 + gcs_3 + gcs_4 + cancer_type_det_2 + 
      ...
      preds <- exp(-(lambda1 * times))
      ex <- as.data.frame(cbind(t = times, St = St2))
      
      
      lines(times, ex$St, type = "l", xlab = "Time (Years)", ylab = "Survival Probability", col = 'blue', lwd = 3)
    ...
    if (input$show_expo_rms) {
      ...
      expo <- psm((Surv(Time,event) ~ age + tumor_size + lnep + gcs_2 + gcs_3 + gcs_4 + cancer_type_det_2 + cancer_type_det_3 + cancer_type_det_4 ), 
                  dist = 'exponential', data = cut_off_dat, x=T, y=T)
      ...
      lines(times, mean_surv_month_expo, col = 'red', lwd = 3)
    ...
    if (input$show_weibull) {
      ...
      lines(times, weib$St, col = 'purple', lwd = 3)
    ...
    if (input$show_weibull_rms) {
      ...
      lines(times, mean_surv_month_wei.rms, col = 'orange', lwd = 3)
    ...
    if (input$show_gompertz) {
      ...
      b <- exp( m1$models$Gompertz$coefficients[2] + m1$models$Gompertz$coefficients[3]*mean(age) +
      ...
      # shape parameter 
      a <- m1$models$Gompertz$coefficients[1]
      a
      ...
      lines(t, gomp_survivor_func, col = 'cyan', lwd = 2)
    ...
    if (input$show_loglogistic) {
      ...
      lines(times, log.logis$St, col = 'green', lwd = 3)
    ...
    if (input$show_loglogistic_rms) {
      ...
      lines(times, mean_surv_month_log.log.rms, col = 'lightblue', lwd = 3)
    ...
    if (input$show_lognormal_rms) {
      ...
      lines(times, mean_surv_month_log.norm.rms, col = 'darkred', lwd = 3)
    ...
    if (input$show_expo || input$show_expo_rms || input$show_weibull || input$show_weibull_rms || input$show_gompertz || input$show_loglogistic || input$show_loglogistic_rms || input$show_lognormal_rms)  {
      legend("topright", legend = c("Exponential - Survival/survHE", "Exponential - RMS", "Weibull (AFT) - RMS",
             ...
             col=c("blue","red","purple","orange","cyan",
